...
interface LeadData {
  ...
  assessment: {
    ...
    rating: string | null;
    // Dynamic question answers grouped by section
    answers: QuestionAnswer[];
    // Score breakdown by question
    scoreBreakdown: Record<string, { questionText: string; score: number; maxScore: number }> | null;
  } | null;
  documents: Array<{
    id: string;
    fileName: string;
...
export default function ClientDashboard({ lead }: { lead: LeadData }) {
  ...
  // ─── Derived Real Data from Dynamic Questions ──────────────────────────────

  const score = a?.percentage ? Math.round(a.percentage) : 0;
  const ratingLabel = a?.rating ? a.rating.replace(/_/g, " ") : "Not Rated";
  const answers = a?.answers ?? [];

  // Helper to find answer by section or display number pattern
  const findAnswer = (pattern: string) =>
    answers.find(
      (ans) =>
        ans.questionText.toLowerCase().includes(pattern.toLowerCase()) ||
        ans.displayNumber === pattern
    );

  // Extract financial data from dynamic questions (by looking for turnover/EBITDA questions)
  const getNumberValue = (pattern: string): number | null => {
    const ans = findAnswer(pattern);
    if (ans && ans.inputType === "number") {
      const val = parseFloat(ans.answerValue);
      return isNaN(val) ? null : val;
    }
    return null;
  };

  // Financial data - look for turnover/EBITDA related questions
  const financials: Financial[] = answers
    .filter(
      (ans) =>
        ans.inputType === "number" &&
        (ans.questionText.toLowerCase().includes("turnover") ||
          ans.questionText.toLowerCase().includes("revenue"))
    )
    .slice(0, 3)
    .map((ans, i) => ({
      year: `Year ${i + 1}`,
      revenue: ans.answerValue || "0",
      profit: "N/A",
      ebitda: "N/A",
    }));

  // Fallback if no financial questions found
  if (financials.length === 0) {
    financials.push(
      { year: "Year 1", revenue: "0", profit: "N/A", ebitda: "N/A" },
      { year: "Year 2", revenue: "0", profit: "N/A", ebitda: "N/A" },
      { year: "Year 3", revenue: "0", profit: "N/A", ebitda: "N/A" }
    );
  }

  const latestRevenue =
    financials.length > 0 && financials[0].revenue !== "0"
      ? `₹${financials[0].revenue} Cr`
      : "N/A";
  const revenueGrowth = "Latest year";

  // Build checklist from boolean questions (where answer is Yes/No)
  const checklist: ChecklistItem[] = answers
    .filter((ans) => ans.inputType === "boolean")
    .map((ans, i) => ({
      id: i + 1,
      label: ans.questionText,
      done: ans.answerValue === "Yes" || ans.answerValue === "true",
      category: ans.section,
    }));

  // Fallback checklist if empty
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        ...
        >
          ...
          >
            {a?.totalScore?.toFixed(1) ?? "0"} / {a?.maxScore?.toFixed(1) ?? "100"} points
          </div>
        </div>
      ...
      {a && answers.filter((ans) => ans.inputType === "number").length > 0 && (
        <div style={card}>
          <div
            style={{
              fontSize: 13,
          ...
          >
            {answers
              .filter((ans) => ans.inputType === "number")
              .slice(0, 6)
              .map((ans) => ({
                label: ans.questionText.length > 30
                  ? ans.questionText.slice(0, 30) + "..."
                  : ans.questionText,
                value: ans.answerValue
                  ? `${ans.unit === "crores" ? "₹" : ""}${ans.answerValue}${ans.unit ? ` ${ans.unit}` : ""}`
                  : "N/A",
              }))
              .map((item, i) => (
              <div
  ...
  const downloadReportPDF = () => {
    ...
    import("jspdf").then(({ default: jsPDF }) => {
      ...
      y += 2;
      addText(`Overall Score: ${a.totalScore?.toFixed(1) ?? "0"}/${a.maxScore?.toFixed(1) ?? "100"}`, 11);
      addText(`Percentage: ${score}%`, 11);
      addText(`Rating: ${ratingLabel}`, 11);
      y += 4;
      addLine();

      // Financial Overview (from dynamic number questions)
      const numberAnswers = answers.filter((ans) => ans.inputType === "number");
      if (numberAnswers.length > 0) {
        addText("Financial Overview", 13, true);
        y += 2;
        numberAnswers.slice(0, 8).forEach((ans) => {
          const val = ans.answerValue
            ? `${ans.unit === "crores" ? "Rs " : ""}${ans.answerValue}${ans.unit ? ` ${ans.unit}` : ""}`
            : "N/A";
          addText(`${ans.displayNumber}. ${ans.questionText}: ${val}`, 11);
        });
        y += 4;
        addLine();
      }
      ...
      answers.forEach((ans, i) => {
        checkNewPage();
        const answerText = ans.inputType === "boolean"
          ? (ans.answerValue === "Yes" ? "Yes" : "No")
          : ans.answerValue || "N/A";
        addText(`${ans.displayNumber || i + 1}. ${ans.questionText}`, 10, true);
        addText(`   Answer: ${answerText} | Score: ${ans.score}/${ans.maxScore}`, 9, false, [80, 80, 80]);
        y += 4;
      });
  ...
  const renderReport = () => (
    <div style={card}>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: t.textMuted, marginTop: 4 }}>
          {lead.companyName} • Score: {a?.totalScore?.toFixed(1) ?? "0"}/{a?.maxScore?.toFixed(1) ?? "100"} ({score}%)
        </div>
        <div
          style={{
            marginTop: 8,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {/* Group answers by section and calculate section scores */}
        {(() => {
          const sections = [...new Set(answers.map((ans) => ans.section))];
          return sections.map((section) => {
            const sectionAnswers = answers.filter((ans) => ans.section === section);
            const totalScore = sectionAnswers.reduce((sum, ans) => sum + ans.score, 0);
            const maxScore = sectionAnswers.reduce((sum, ans) => sum + ans.maxScore, 0);
            const scorePercent = maxScore > 0 ? Math.round((totalScore / maxScore) * 100) : 0;
            return { label: section, score: scorePercent };
          });
        })().map((s, i) => {
          const color =
